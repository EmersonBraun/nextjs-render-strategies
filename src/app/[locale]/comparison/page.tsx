...
function getDynamicColor(dynamic: string, dynamicLabels: Record<string, string>) {
...
function getServerLoadColor(load: string, serverLoadLabels: Record<string, string>) {
...
export default async function ComparisonPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const headers = t.raw('headers') as any
  ...
  const takeaways = t.raw('takeaways') as any
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b">
                  <th className="text-left p-3 font-semibold">{headers.mode}</th>
                  <th className="text-left p-3 font-semibold">{headers.initialSpeed}</th>
                  <th className="text-left p-3 font-semibold">{headers.seoPerformance}</th>
                  <th className="text-left p-3 font-semibold">{headers.dynamicContent}</th>
                  <th className="text-left p-3 font-semibold">{headers.serverLoad}</th>
                  <th className="text-left p-3 font-semibold">{headers.useCases}</th>
                </tr>
              </thead>
              <tbody>
                {comparisonData.map((mode) => (
                  <tr key={mode.mode} className="border-b hover:bg-muted/50">
                    ...
                    <td className={`p-3 font-medium ${getSpeedColor(mode.initialSpeed, speedLabels)}`}>
                      {mode.initialSpeed}
                    </td>
                    <td className={`p-3 font-medium ${getSEOColor(mode.seo, seoLabels)}`}>
                      {mode.seo}
                    </td>
                    <td className={`p-3 font-medium ${getDynamicColor(mode.dynamicContent, dynamicLabels)}`}>
                      {mode.dynamicContent}
                    </td>
                    <td className={`p-3 font-medium ${getServerLoadColor(mode.serverLoad, serverLoadLabels)}`}>
                      {mode.serverLoad}
                    </td>
                    <td className="p-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 border rounded-lg">
              ...
              <ul className="text-sm space-y-1">
                {takeaways.static.items.map((item: string, index: number) => (
            ...
            <div className="p-4 border rounded-lg">
              ...
              <ul className="text-sm space-y-1">
                {takeaways.dynamic.items.map((item: string, index: number) => (
                  <li key={index}>• {item}</li>
                ))}
              </ul>
            </div>
            <div className="p-4 border rounded-lg">
              ...
              <ul className="text-sm space-y-1">
                {takeaways.hybrid.items.map((item: string, index: number) => (
                  <li key={index}>• {item}</li>
                ))}
              </ul>
            </div>
